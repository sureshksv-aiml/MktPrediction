...
## Context
React's `MutableRefObject<T>` type is deprecated in favor of `RefObject<T>`. The key difference:
- `MutableRefObject<T>` has `current: T` (never null)
- `RefObject<T>` has `current: T | null` (can be null)

Using deprecated `MutableRefObject<T>` results in TypeScript warnings and should be replaced with `RefObject<T>` with proper null handling.
...
In some cases, you know the ref will always have a value. You can use:
